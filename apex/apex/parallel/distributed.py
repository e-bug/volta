...
class Reducer(object):
    ...
    :class:`Reducer` is designed to work with the upstream launch utility scripts
    ...
    It also assumes that your scripts calls ``torch.cuda.set_device(args.rank)`` before creating the model.
...
class DistributedDataParallel(Module):
    ...
    :class:`DistributedDataParallel` is designed to work with the upstream launch utility scripts
    ``torch.distributed.launch`` with ``--nproc_per_node <= number of gpus per node``.
    ...
    def allreduce_fallback(self):
        ...
        # training scripts, and overwritten in the next forward pass.  So it's harmless.
        if self.retain_allreduce_buffers:
