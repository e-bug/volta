...
DIR2FILE = {
        'training': 'Train_GCC-training.tsv',
...
def load_image_ids(img_root, split_dir, group_id, total_group):
    ...
    fname = os.path.join(img_root, DIR2FILE[split_dir])
    df = open_tsv(fname, split_dir)
    total_num = len(df)
    ...
    for i, img in enumerate(current_df.iterrows()):
        ...
        filepath = os.path.join(img_root, im_name)
        pathXid.append((filepath, image_id))
