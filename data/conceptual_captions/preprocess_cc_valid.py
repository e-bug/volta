...
class Conceptual_Caption(RNGDataFlow):
    ...
    def __init__(self, corpus_path, shuffle=False):
        ...
        self.name = os.path.join(corpus_path, 'imgfeats/valid_obj36-36.tsv')
        self.infiles = [self.name]
        self.counts = []

        self.captions = {}
        df = open_tsv(os.path.join(corpus_path, 'Validation_GCC-1.1.0-Validation.tsv'), 'validation')
        ...
        with open(os.path.join(corpus_path, 'annotations/caption_valid.json')) as f:
...
if __name__ == '__main__':
    ...
    LMDBSerializer.save(ds1, os.path.join(corpus_path, 'imgfeats/volta/validation_feat_all.lmdb'))
