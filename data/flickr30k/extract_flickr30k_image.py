...
def load_image_ids(img_root, split_name, group_id, total_group):
    """images in the same directory are in the same split"""
    fnames = []
    if split_name == 'flickr30k':
        lst = os.listdir(os.path.join(img_root, 'images'))
        for i in lst:
            if i.endswith('.jpg'):
                fnames.append({'filename': i})
    else:
        fname = os.path.join(img_root, 'dataset_flickr30k.json')
        lst = json.load(open(fname))['images']
        for e in lst:
            if e['split'] == split_name:
                fnames.append(e)
    print('%s: %d / %d' % (split_name, len(fnames), len(lst)))
    ...
    pathXid = []
    for e in current_lst:
        name = e['filename']
        imgid = int(e['filename'].split('.')[0])
        filepath = os.path.join(img_root, 'images', name)
        pathXid.append((filepath, imgid))
...
if __name__ == '__main__':
    ...
    args.outfile = "%s_obj%d-%d.tsv.%d" % (args.split, MIN_BOXES, MAX_BOXES, args.group_id)
    ...
    image_ids = load_image_ids(args.imgroot, SPLIT2NAME[args.split], args.group_id, args.total_group)
