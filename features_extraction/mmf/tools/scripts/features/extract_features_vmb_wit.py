...
#                f.write(str(begin_idx+e) + '\n')
    ...
    def extract_features(self):
        ...
        dfs = pd.read_csv(os.path.join(image_dir, 'train_image_pixels_avail.tsv'), chunksize=150_000, sep='\t', usecols=['base64_image'])
        for ix, df in enumerate(dfs):
            if ix == self.args.chunk:
                break
...
#        os.makedirs(out_dir, exist_ok=True)
        ...
        start_idx = self.args.chunk * 150000
#        files = [os.path.join(out_dir, 'wit_%d' % (self.start_idx + idx) for idx in range(len(images))]
        files = ['wit_%d' % (start_idx + idx) for idx in range(len(images))]
